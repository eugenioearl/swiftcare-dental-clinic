...
import {
  ...
  Clock, Activity, FileText, UserCheck, TrendingUp, HeartHandshake,
...
export function OptimizedMobileNavigation({ className }: OptimizedMobileNavProps) {
    ...
    switch (userRole) {
        ...
        return [
          ...
          { 
            ...
            children: [
              ...
              { icon: Stethoscope, label: 'Dental Chart', href: '/admin/chart' },
              { icon: Stethoscope, label: 'Treatment', href: '/admin/treatment' }
            ]
          },
          { 
            icon: Zap, 
      ...
      case 'staff':
        return [
          ...
          { 
            ...
            label: 'Patients', 
            children: [
              { icon: Users, label: 'Patient List', href: '/staff/patients' },
              { icon: ClipboardList, label: 'Forms', href: '/staff/forms' }
            ]
          },
        ]
      ...
      default: // patient
        return [
          ...
          { icon: UserCheck, label: 'Check-In', href: '/patient/check-in' },
        ...
        <div key={item.label} className="space-y-1">
          ...
          >
            <CollapsibleTrigger asChild>
              <button className={cn(
                'mobile-list-item flex items-center gap-3 mx-4 w-full text-left transition-all duration-200',
                'text-gray-700 hover:bg-gray-50 border-gray-200'
              )}>
                <item.icon className="w-5 h-5 text-gray-500" />
                <span className="font-medium flex-1">{item.label}</span>
                {item.badge && (
                  <Badge className="text-xs px-2 py-0.5 bg-primary text-white">
                    {item.badge}
                  </Badge>
                )}
                {isOpen ? (
  ...
  return (
    <>
      ...
      >
        ...
        <div className="bg-gradient-to-r from-primary-500 to-secondary-500 p-6 text-white">
          ...
          {session?.user && (
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold">{formatDisplayName(session.user.firstName, session.user.lastName)}</p>
        ...
        {/* Navigation Items */}
        <div className="flex-1 py-4 overflow-y-auto max-h-[calc(100vh-200px)]">
...
interface OptimizedMobileBottomNavProps {
  className?: string
}

export function OptimizedMobileBottomNav({ className }: OptimizedMobileBottomNavProps) {
  const { data: session } = useSession()
  const { shouldUseMobileLayout } = useBreakpoint()
  const pathname = usePathname()
      ...
      {quickActions.map((action, index) => {
        ...
        return (
          <Link
            ...
            href={action.href}
            className={cn(
              'touch-target flex flex-col items-center justify-center py-2 transition-colors duration-200 relative',
              isActive 
